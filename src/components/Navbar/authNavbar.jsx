...
function AuthNavbar({ onLogout }) {
  ...
  return (
    <nav className="flex flex-wrap items-center justify-between px-4 py-1 bg-gradient-to-r from-white to-pink-950  fixed top-0 w-full z-50">
      <div className="flex items-center flex-shrink-0 text-white mr-6">
        <Link to="/" className="flex items-center">
          <img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcThHcIOXPXpy-MePiSIDNImw3PJlXUucl5-0A&usqp=CAU" alt="" className="h-15 w-20 mr-2" />
          <span className="text-xl font-bold tracking-tight underline text-blue-700"> Your are logged In...</span>
        </Link>
      </div>
      ...
      <div className={`w-full block flex-grow lg:flex lg:items-center lg:w-auto ${showMenu ? "block" : "hidden"} lg:block mt-4 lg:mt-0`}>
        ...
        <div className="flex items-center mt-4 lg:mt-0 lg:ml-auto">
          <button
            ...
            className=" hover:text-cyan-300 text-white font-bold py-2 px-4  mr-4"
          >
            Logout
          </button>
          ...
          <Link to="/profile" className="mr-4">
            <IoPersonSharp className="w-8 h-8 rounded-full text-cyan-300" />
          </Link>
          <Link to="/cart" className="relative text-cyan-300">
